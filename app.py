...
#mengambil file picke dan load model NN
file = open("NeuralNetwork/feature.pickle",'rb')
feature_file_from_nn = pickle.load(file)
file.close()

model_file_from_nn = load_model("NeuralNetwork/modelNN.h5")

#mengambil file picke dan load model LSTM
file = open("LSTM/x_pad_sequencesLSTM.pickle",'rb')
feature_file_from_lstm = pickle.load(file)
file.close()

model_file_from_lstm = load_model("LSTM/modelLSTM_new.h5")

#nn
@swag_from("yaml/nn.yml", methods=['POST'])
@app.route('/nn-teks', methods=['POST'])
...
def nn():
    ...
    json_response = {
        'status_code' : 200,
        'description' : "Result of Analysis Using nn",
        'data' : {
            'text': text,
...
@swag_from("yaml/nn_file.yml", methods=['POST'])
